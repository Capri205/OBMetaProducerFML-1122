...
public class OBMetaTask {
	...
	public void run(String metafile, Boolean obsfucate, Integer range) {
		...
		if (scm.getPlayers().size() != 0) {
			pw.print("["); int i=0;
			for (EntityPlayerMP player : scm.getPlayers()) {
				i++;
				...
				playerWorld = player.world.getWorldInfo().getWorldName();
				if (player.world.getWorldInfo().getWorldName().equals("DIM"+player.dimension)) {
				...
				pw.print(	"{\"msg\":\""+player.getDisplayNameString()+"\","+
						...
						"\"y\":\""+y+"\","+
						"\"z\":\""+z+"\"}");
				if (i < scm.getPlayers().size()-1 ) { pw.print(",");}				
				pw.print("]");
			}
		} else {
			pw.print("[]");
